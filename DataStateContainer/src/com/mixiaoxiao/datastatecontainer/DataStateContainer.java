...
public class DataStateContainer extends ViewGroup implements OnWrapperScrollListener, OnClickListener {
	...
	public static final int DEFAULT_REFRESH_OR_LOADVIEW_EXACT_HEIGHT_PX = (int) (Resources.getSystem()
			.getDisplayMetrics().density * 48f * 0.5f);

	private static final Interpolator sInterpolator = new DecelerateInterpolator(2f);
	private static final CallBack sFakeCallBack = new DataStateInterface.CallBackAdapter();
	...
	// copy from supportv4 SwipeRefreshLayout
	...
	/** 先初始化一个FakeCallBack **/
	private CallBack mCallBack = sFakeCallBack;

	public void setCallBack(CallBack callBack) {
		mCallBack = callBack;
		final boolean clickToLoad = !mCallBack.autoLoadWhenScrollToLastItem();
		mLoadView.onLoadIdle(mCallBack.hasMoreDataToLoad(), clickToLoad, false);
	}
	...
	public boolean onEmpty(boolean empty, boolean retry) {
	...
	/**
	 * STATE_REFRESH_ING和STATE_LOAD_ING本方法会处理View状态 
	 * 其他STATE需要自行处理View状态
	 * @param newState
	 */
...
//			log("setStateInternal->" + mState);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			//修正一下，使得overscrollTop必须 >= 0
			final float overscrollTop = Math.max(0, (y - mInitialMotionY) * DRAG_RATE);
			if (mIsBeingDragged) {
...
//				}
		...
		case MotionEvent.ACTION_CANCEL: {
			...
			if (overscrollTop > mTotalDragDistance) {
				...
				mScroller.startScroll(0, currScrollY, 0, Math.round(-currScrollY - mTotalDragDistance),
						ANIMATE_DURATION);
			} else {
				setStateInternal(STATE_IDLE);
				mScroller.startScroll(0, currScrollY, 0, -currScrollY, ANIMATE_DURATION);
	...
	public void onLastChildScrolled(int bottomSpace, boolean scrollStateIdle) {
		...
		// MathUtil.limitInt(bottomSpace, 0, mLoadView.getExactlyHeight());
		if(bottomSpace < 0 ){
			bottomSpace = 0;
		}else if(bottomSpace > mLoadView.getExactHeight()){
			bottomSpace = mLoadView.getExactHeight();
		}
		setLoadViewTranslationY(-bottomSpace);
		// final int newLoadViewTop = getHeight() - space;
		// mLoadView.getView().offsetTopAndBottom(newLoadViewTop -
		// mLoadView.getView().getTop()) ;
		// invalidate();
	}
	...
	public void onLastChildVisibilityChanged(boolean visible) {
		...
		if (visible) {
			// 当LastChild刚刚显示的时候刷新一下LoadView的状态
			if (mState == STATE_IDLE){
				final boolean clickToLoad = !mCallBack.autoLoadWhenScrollToLastItem();
				mLoadView.onLoadIdle(mCallBack.hasMoreDataToLoad(), clickToLoad, false);
			}
	...
	public void onLastChildTerminalChanged(boolean terminal) {
		log("onLastChildTerminalChanged terminal=" + terminal);
		if (terminal) {// terminal=true表示上拉到达了终点
			if (mState == STATE_IDLE) {
				final boolean hasMoreDataToLoad = mCallBack.hasMoreDataToLoad();
				final boolean autoLoadWhenScrollToLastItem = mCallBack.autoLoadWhenScrollToLastItem();
				if (hasMoreDataToLoad && autoLoadWhenScrollToLastItem) {
					setStateInternal(STATE_LOAD_ING);
				} else {// 非自动加载或没有数据的情况，需要clickToLoad
					final boolean clickToLoad = true;
					mLoadView.onLoadIdle(hasMoreDataToLoad, clickToLoad, false);
				}
			}
		} else {// 没有到达终点
...
//				setStateInternal(STATE_IDLE);
//				mLoadView.onLoadIdle(mCallBack.hasMoreDataToLoad(), clickToLoad, false);
				//NO NO NO 从正在加载->“上拉加载”或“点击加载”状态转换是由onLoadEnd(boolean success)处理的
			} else if (mState == STATE_IDLE) {
				mLoadView.onLoadIdle(mCallBack.hasMoreDataToLoad(), clickToLoad, false);
			}
		}
